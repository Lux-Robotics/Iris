...
def add_camera_controls(win_name, cap):
    ...
    if cap.manual_exposure:
        ...
        cv2.createTrackbar(
            ...
            exposure,
            1000,
            lambda v: cap.set(cv2.CAP_PROP_EXPOSURE, v / 1000),
        )


def main():
    parser = argparse.ArgumentParser(
        description="Interactive camera calibration using efficient pose selection"
    )
    parser.add_argument(
        "-c",
        "--config",
        help="path to calibration configuration (e.g. data/calib_config.yml)",
    )
    parser.add_argument(
        "-o",
        "--outfile",
        help="path to calibration output (defaults to calib_<cameraId>.yml)",
    )
    parser.add_argument(
        "-m",
        "--mirror",
        action="store_true",
        help="horizontally flip the camera image for display",
    )
    ...
    if live:
        ...
        cap = cv2.VideoCapture(
            cfg.getNode("images").string() + "frame%0d.png", cv2.CAP_IMAGES
        )
        wait = 0
        assert cap.isOpened()
