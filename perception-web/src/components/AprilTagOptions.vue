<template>
  <v-card elevation="12" min-width="400" rounded="lg">
    ...
    <v-card-text>
      ...
      <v-slider v-model="decimate" hide-details :max="6" :min="1" :step="1">
        <template #label>
          <span class="options-label">Decimate</span>
        </template>
        <template #append>
          <v-text-field v-model="decimate" density="compact" hide-details style="width: 80px" type="number"
            variant="outlined" />
        </template>
      </v-slider>
      <v-slider v-model="blur" class="my-4" hide-details :max="4" :step="0.1">
        <template #label>
          <span class="options-label">Gaussian Blur</span>
        </template>
        <template #append>
          <v-text-field v-model="blur" density="compact" hide-details style="width: 80px" type="number"
            variant="outlined" />
        </template>
      </v-slider>
      <v-slider v-model="nThreads" class="my-4" hide-details :max="8" :min="1" :step="1">
        <template #label>
          <span class="options-label">CPU Threads</span>
        </template>
        <template #append>
          <v-text-field v-model="nThreads" density="compact" hide-details style="width: 80px" type="number"
            variant="outlined" />
        </template>
      </v-slider>
