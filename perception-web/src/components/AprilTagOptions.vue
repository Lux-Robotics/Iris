<template>
  <v-card elevation="12" rounded="lg" min-width="350">
    ...
    <div class="px-4 py-4">
      ...
      <v-slider v-model="decimate" :max="3" :step="0.5" class="ma-4" label="Decimate" hide-details>
        <template v-slot:append>
          <v-text-field v-model="decimate" density="compact" style="width: 80px" type="number" variant="outlined"
            hide-details></v-text-field>
        </template>
      </v-slider>
      <v-slider v-model="blur" :max="4" :step="1" class="ma-4" label="Gaussian Blur" hide-details>
        <template v-slot:append>
          <v-text-field v-model="blur" density="compact" style="width: 80px" type="number" variant="outlined"
            hide-details></v-text-field>
        </template>
      </v-slider>
      <v-slider v-model="nthreads" :max="8" :step="1" class="ma-4" label="CPU Threads" hide-details>
        <template v-slot:append>
      ...
      <v-switch label="Refine Edges" class="ma-4" hide-details></v-switch>
    </div>
  </v-card>
</template>
