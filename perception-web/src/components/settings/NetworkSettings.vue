  ...
  <v-card variant="elevated">
    <template #text>
      ...
      <span>This is used to determine the address of the NetworkTables instance to publish results to. Alternatively,
        enter an IP address or hostname if you are not using Perception with a roboRio.</span>
      <v-spacer class="my-2" />
      <v-text-field v-model="robotServerIP" density="compact" :rules="rules" variant="outlined" @input="validate">
        <template #append-inner>
          <v-slide-y-transition>
            <v-btn v-if="robotServerIP !== robotServerIPRef" class="text-capitalize" color="secondary" density="compact"
              :disabled="!backendConnected || !ipValid" slim text="save" variant="tonal" @click="updateServerIP" />
          </v-slide-y-transition>
        </template>
      </v-text-field>
      ...
      <div class="edit-settings">
        <div>
          <h3>Hostname</h3>
          <span>
            PeninsulaPerception is using the hostname "perception.local". This will be the camera name sent to the robot
            and
            you can access this dashboard at "http://perception.local:5800"
          </span>
        </div>
      ...
      <div class="edit-settings">
        <div>
          ...
          <span>
            PeninsulaPerception uses a dynamic IP address by default. However, a static IP address reduces boot time and
            leads
